...
def report(report, test_result):
    ...
    report.add_description(
        f"""Self-test is a diagnostic testing sequence that tests the integrity and functionality of
        ...
        an extended self-test. This test verifies the extended self-test.
        <br/><br/>

        The self-test diagnostic is run standalone and concurrent with a light IO workload.  In
        both cases the diagnostic must pass, complete within the run time limit, and report progress
        in Log Page 6 that is monotonic and roughly linear.
        <br/><br/>

        The extended self-test run time limit is from the Extended Device Self-test Time (EDSTT)
        field.  The NVMe specification states EDSTT “this field indicates the nominal amount of time
        in one-minute units that the controller takes to complete an extended device self-test
        operation when in power state 0”.   There is no maximum run time value defined in the
        specification therefore it is possible for an extended self-test to take longer than EDSTT
        and still comply with the specification.  However, this test uses the EDSTT as the run time
        limit. The EDSTT field is {data['runtime limit']} minutes for this NVMe drive.
        <br/><br/>
